...
frappe.ui.form.on("Inspection Improvement", {
	refresh(frm) {
		if (!frm.doc.__islocal) {
			frm.add_custom_button(__('Checklist'), function () {
				frappe.db.get_value("Maintenance Checklist", { "inspection_improvement": frm.doc.name}, ["name"])
				.then(function (r) {
					if (!r.message.name) {
						frm.call({
		...
		if (!frm.is_new()) {
			...
			frm.fields_dict.activity_one.grid.add_custom_button(__("Add Multiple"), function () {
				open_image_dialog("Site Activity", "activity_one", frm, activity=true)
			});

			frm.fields_dict.activity_two.grid.add_custom_button(__("Add Multiple"), function () {
				open_image_dialog("Health Safety and Environment", "activity_two", frm, activity=true)
			});
		}
	},
		...
		frm.add_custom_button(__('Download Images'), () => {
			frappe.require([
					...
					fields: [
						...
						},
						{
							...
							click: () => {
								...
								const urls = [...checks].map(input => input.value);
				...
				function downloadImagesAsZip(urls) {
					...
					let completed = 0;
					frappe.show_progress("Preparing download", 0, urls.length);
					...
					urls.forEach((url, idx) => {
						fetch(url)
							...
							.then(blob => {
								...
								completed++;
								frappe.show_progress("Preparing download", completed, urls.length);
				
								if (completed === urls.length) {
									const zipName = `selected_image_${frm.doc.name}.zip`;
	...
	document_template: function(frm) {
		if (frm.doc.document_template) {
			frm.call("add_reference_documents")
		}
	}, 
});


function open_image_dialog(section, fieldname, frm, activity=false) {
	let dialog;
	let image_field =  activity ? "attachment" : "image";
		...
		fields: [
			...
			},
			{
				...
				click: () => {
					...
					if (remaining <= 0 && fieldname !== "additional_image" && !activity) {
					...
					new frappe.ui.FileUploader({
						...
						on_success(file) {
							...
							if (image_rows.length > 4 && fieldname !== "additional_image" && !activity) {
											...
											canvas.toBlob(blob => {
												...
												reader.onload = function(event) {
													...
													frm.call({
														...
														doc: frm.doc,
														args: {
															file_url: image_rows[index][image_field],  // Existing image URL
...
frappe.ui.form.on("Inspection Improvement BOQ", {
